...
export const dueChip = (due) => {
  ...
  if (diff < 0) return { cls: 'text-red-700 bg-red-50 ring-red-200', label: `${Math.abs(diff)}일 경과${day}` };
  if (diff <= 3) return { cls: 'text-orange-700 bg-orange-50 ring-orange-200', label: `D-${diff}${day}` };
  if (diff <= 7) return { cls: 'text-amber-800 bg-amber-50 ring-amber-200', label: `D-${diff}${day}` };
  return { cls: 'text-stone-600 bg-stone-50 ring-stone-200', label: `D-${diff}${day}` };
};
